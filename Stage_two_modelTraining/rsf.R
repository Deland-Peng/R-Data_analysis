...
rsf_param <- function(x.train, fold){
  ...
  #m <- seq(200,1600,200)  #如果数据量大尽量间隔大点，间隔过小没有实际意义 seq(500,2500,500)
  cindex_fin <- c()
  j=1
  best_cindex = 0
  best_ntree = 0
 ...
 # for(ntree in seq(500,2500,500)){   #j指的是随机森林的数量
  for(ntree in seq(60,160,20)){   #j指的是随机森林的数量
    print("ntree")
    print(ntree)
    cindex_rsf_cross <- c()
    for (i in 1:k){
  ...
  model_rsf <- rfsrc(formula=as.formula(Surv(time,status)~.),
                     ...
                     seed=1234,nodesize=3,splitrule = "logrank",tree.err=TRUE,importance = T)
  ...
  #save("result", file = sprintf("Stage2/all_model/result/all_features/%d_rsf_result.RData", fold))
  #save("result", file = sprintf("Stage2/all_model/result/10_features/%d_rsf_result.RData", fold))
  #save("result", file = sprintf("Stage2/all_model/result/17_features/%d_rsf_result.RData", fold))
  #save("result", file = sprintf("Stage2/all_model/result/half_features/%d_rsf_result.RData", fold))
  #save("result", file = sprintf("Stage2/all_model/result/52_features/%d_rsf_result.RData", fold))
  save("result", file = sprintf("Stage2/all_model/result/48_features/%d_rsf_result.RData", fold))
  }